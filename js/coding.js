...
Coding.createNewEditor = function($editorNumber){
   ...
   document.getElementById("editor"+$editorNumber).style.zoom = (1 / App.screenRatio);
            ...
            switch (App.os) {
                ...
                case "ios":
                    ...
                    window.addEventListener('keyboardHeightWillChange', function (event) {
                        var _bottomSpace = (parseInt(event.keyboardHeight) + 100);
                        document.getElementById('keyboard-box').style.bottom = _bottomSpace + "px";
                        // document.getElementById('keyboard-box').style.top = "0px";
                    });
                    */
    ...
    myeditor.selection.on("changeCursor", function() {
        ...
        var miniKeyboardElement = document.getElementById('keyboard-box');
        if (miniKeyboardElement.style.display != "none") {
            miniKeyboardElement.style.top = cursorPixelPosition.top + editorTop + 300 + "px";
        }
        console.log(cursorPixelPosition)
        */
       
        // myeditor.renderer.$cursorLayer.getPixelPosition({row, column}, relativeToScreen);
        // myeditor.getCursorPosition()
        editorCloseMobileMenu();
...
Coding.closeNativeKeyboard = function() {

    // Butona tıklandığında otomatik keyboard kapandığı için koda gerek yok.
    // eval("editor" + Coding.currentEditorNumber + ".blur();");
